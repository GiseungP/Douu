document.addEventListener('DOMContentLoaded', () => {
    ...
    resetBtn.addEventListener('click', async () => {
        if (!confirm('정말로 문장을 초기화하시겠습니까?')) return;
        ...
        try {
            ...
            const data = await response.json();
            if (data.success) {
                sentenceEl.textContent = data.sentence || '문장을 작성하세요...';
            } else {
    ...
    saveBtn.addEventListener('click', async () => {
        try {
            ...
            if (data.success) {
                ...
                alert(`저장 실패: ${data.error || '저장할 문장이 없습니다.'}`);
            }
        } catch (err) {
    ...
    function loadSentence() {
        fetch('/get_sentence')
            ...
            .catch(err => console.error('문장 로드 오류:', err));
    }
